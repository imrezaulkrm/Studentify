pipeline {
    ...
    stages {
        ...
        stage('source code pull from github') {
            steps {
                git branch: 'backend', url: 'https://github.com/imrezaulkrm/Studentify.git'
            }
        }
        ...
        stage('Updating Kubernetes deployment file') {
            steps {
                ...
                sh "cat k8s/springboot-deploy.yml"
                // Construct the sed command to change only line 17
                sh """sed -i '33s#image:.*#image: ${IMAGE_NAME}:${IMAGE_TAG}#' k8s/springboot-deploy.yml"""
                sh "cat k8s/springboot-deploy.yml"
            }
        }
        ...
        stage('Push the changed deployment file to Git') {
            steps {
                script {
                    ...
                    sh 'git add k8s/springboot-deploy.yml'
